...
function getSessionSecret() {
  ...
  if (process.env.NODE_ENV === 'production') throw new Error('Falta configurar APP_SESSION_SECRET.')
...
export function createAppSessionToken(session: Omit<AppSession, 'expiresAt'>, remember: boolean) {
...
export function readAppSessionToken(token?: string | null): AppSession | null {
  ...
  try {
    ...
    if (session.userType !== 'ADMINISTRADOR' && session.userType !== 'ESTUDIANTE') return null
    if (session.userType === 'ADMINISTRADOR' && !session.authUserId) return null
    if (session.userType === 'ESTUDIANTE' && !session.documentNumber) return null
    return session
  } catch {
    return null
  }
